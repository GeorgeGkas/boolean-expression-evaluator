...
import { BinaryExpressionTree, BinaryExpressionTreeNodeIdType, BinaryExpressionTreeNodeType, Operation } from "./BinaryExpressionTree";
...
const GenericOptions = (props: {
  ...
  <>
    ...
    {Object.keys(props.args).length && <option value={Operation.ARGUMENT}>argument</option>}
...
const ArgsOptions = (props: {
  args: Args,
}) => {
  return <>
  {
    Object.entries(props.args).map(([argName], id) =>  (
      <option key={id} value={`${Operation.ARGUMENT}-${argName}`}>{argName}</option>
    ))
  }
  </>
}
...
const normalizeNodeValue = (type: Operation, value: string | boolean | Set<BinaryExpressionTreeNodeIdType>, args: Args) => {
  console.log(value)
...
export function OperationBuilder(props: {
  args: Args,
  binaryExpressionTreeNodes: BinaryExpressionTreeNodeType<Set<BinaryExpressionTreeNodeIdType>>[],
  binaryExpressionTree: BinaryExpressionTree,
  updateBinaryExpressionTreeNodes: () => any
}): JSX.Element {
  const isNodeTypeConjunctionOrDisjunction = (type: Operation) => type === Operation.CONJUNCTION || type === Operation.DISJUNCTION
  const changeNodeType = (nodeId: BinaryExpressionTreeNodeIdType) => (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    } else { // targetValue.startsWith(Operation.GENERIC)
      normalizedType = Operation.GENERIC
    }

 
    props.binaryExpressionTree.changeNodeTypeById(nodeId, normalizedType, normalizedValue)
  ...
  React.useEffect(() => {
    for (const node of props.binaryExpressionTreeNodes) {
      if (normalizeNodeValue(node.type, node.value, props.args) === 'Argument-Argument') {
        props.binaryExpressionTree.changeNodeValueById(node.id, Object.keys(props.args).shift()!)
        props.updateBinaryExpressionTreeNodes()
        return
      }
    }
    ...
    <div className="operation-builder-wrapper">
      {
        props.binaryExpressionTreeNodes.map((node, idx) => {
          const normalizedNodeValue = normalizeNodeValue(node.type, node.value, props.args)
          return (
            <section className="operation-node" key={idx} style={{
              marginLeft: `${node.level * 30}px`
            }}>
              <div>
                <select value={normalizedNodeValue} onChange={changeNodeType(node.id)}>
                  <BuildOperationOptions type={node.type} args={props.args}/>
                </select>
                <button className="operation-node-remove-button" onClick={removeNode(node.id)}>✖</button>
              </div>
              <div>
                <button 
                className={`operation-node-add-op-button ${!isNodeTypeConjunctionOrDisjunction(node.type) ? 'hidden' : ''}`}
                onClick={addOperation(node.id)}>+ add op</button>
              </div>
            </section>
          )
        })
